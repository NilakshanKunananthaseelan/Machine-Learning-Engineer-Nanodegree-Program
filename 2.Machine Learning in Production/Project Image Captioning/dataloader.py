...
class CaptionDataset(data.Dataset):
    ...
    def __init__(self,transform,mode,batch_size,
                ...
                annotations_file,load_vocab,image_dir,is_flickr):
        self.transform = transform
        self.mode = mode
        self.batch_size = batch_size
        self.vocab = Vocabulary(threshold,file,start_seq,end_seq,unk_word,annotations_file,load_vocab,is_flickr)
        self.image_dir = image_dir
        self.sample_size = sample_size
        self.is_flickr=is_flickr
        print(self.is_flickr)
        
        if mode in ['train','val']:
            self.coco = COCO(annotations_file,self.is_flickr)
            self.ids = list(self.coco.anns.keys())
            print("IDS",len(self.ids))
            print(len(set(self.ids)))
...
#             self.paths = os.listdir('val2014')
    ...
    def __getitem__(self,index):
        ...
        if self.mode in ['train','val']:
            ...
            if self.is_flickr:
                img_path = self.coco.loadImgs(img_id)['file_name']
...
def get_loader(transform,
    ...
    if mode =='train':
        ...
        if is_flickr:
            ...
            annotations_file = os.path.join('Flickr8k_train.json')
    
    if mode =='val':
        assert load_vocab==True,"Vocab must be loaded from disk"
        assert os.path.exists(file),"File does not exist"
        if is_flickr:
            img_dir = os.path.join('val')
            annotations_file = os.path.join('Flickr8k_val.json')
    ...
    if mode =='test':
        ...
        if is_flickr:
            ...
            annotations_file = os.path.join('Flickr8k_test.json')
